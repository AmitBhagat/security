	...
	public Optional<OrderResponse> buildResponse(Order order) throws BussinessException {
		if(!(order==null)) {
			...
			if(deliveryAddress !=null) {
				...
				String line2= deliveryAddress.getAddressLine2();
				...
				String address = Arrays.asList(line1,line2,city,state,zipcode,country).stream().filter(item->  (item !=null && !item.equals(""))).collect(Collectors.joining(","));
			response.setDeliveryAddress(address);
			}
			int restaurantId = order.getRestauantId();
			Restaurant restaurant = (Restaurant) restaurantService.getRestaurantById(restaurantId).getData();
			if(restaurant !=null) {
				response.setRestaurantName(restaurant.getRestaurantName());
				String line1= restaurant.getAddressLine1();
				String line2= restaurant.getAddressLine2();
				String city= restaurant.getCity();
				String country= restaurant.getCountry();
				String restaurantAddress = Arrays.asList(line1,line2,city,country).stream().filter(item-> (item !=null && !item.equals(""))).collect(Collectors.joining(","));
				response.setRestaurantAddress(restaurantAddress);
			}
